        ...
        uint32_t devices, int *format, uint32_t *channels, uint32_t *sampleRate, status_t *status)
{
    ...
    { // scope for the lock
...
#ifdef QCOM_VOIP_ENABLED
        ALOGE(" AudioHardware::openOutputStream mOutput=%p flags=0x%x", mOutput, flags);
        ...
        if (mOutput && !((flags & AUDIO_OUTPUT_FLAG_DIRECT) && (flags & AUDIO_OUTPUT_FLAG_VOIP_RX))
            ...
            return 0;// mOutput; // workaround jb AudioFlinger
        }
...
void AudioHardware::closeOutputStream(AudioStreamOut* out) {
    Mutex::Autolock lock(mLock);
    ALOGE(" AudioHardware::closeOutputStream out=%p", out);
    if ((mOutput == 0
#ifdef QCOM_VOIP_ENABLED
        && mDirectOutput == 0
#endif
        ...
        AudioSystem::audio_in_acoustics acoustic_flags)
{
    ...
    if (!audio_is_input_device((audio_devices_t)devices)) {
        return 0;
    }
