...
#endif
#ifdef QCOM_ANC_HEADSET_ENABLED
       if (mAvailableOutputDevices & AUDIO_DEVICE_OUT_ANC_HEADSET) {
           device |= AUDIO_DEVICE_OUT_ANC_HEADSET;
       }
       if (mAvailableOutputDevices & AUDIO_DEVICE_OUT_ANC_HEADPHONE) {
           device |= AUDIO_DEVICE_OUT_ANC_HEADPHONE;
       }
#endif
       // Do not play media stream if in call and the requested device would change the hardware
       // output routing
       if (mPhoneState == AudioSystem::MODE_IN_CALL &&
          !audio_is_a2dp_device((AudioSystem::audio_devices)device) &&
...
audio_devices_t AudioPolicyManager::getDeviceForInputSource(int inputSource)
{
    ...
    ALOGV("getDeviceForInputSource() inputSource=0x%x mAvailableInputDevices=0x%x", inputSource, mAvailableInputDevices);
    switch(inputSource) {
